 ...
 *
 */
public class DetailPageCommonQueryer extends  BaseCommonDBQueryer{

	
	private final static Logger logger = Logger.getLogger(DetailPageCommonQueryer.class);
	
	@Override
	public TableQueryResult queryTableQueryResult(long showTableConfigId,
			QueryCondition<Object[]> condition) {
		
		logger.debug("=====go into DetailPageCommonQueryer.queryTableQueryResult======");
...
//							if(rs.getObject(itemDC.getFieldNameAlias()) != null){
								...
								DetailPageCommonQueryer editPageCommonQueryer=new DetailPageCommonQueryer();
								...
								//递归查询
								//TableQueryResult tableQueryResult = queryTableQueryResult(p_ShowTableConfig.getId(),p_condition);
									...
									if(itemDC.getOldValue() != null){
										...
										for(Map<String, Object> map:list){//遍历查询到的记录
											...
											if(map.get(key) != null){
												...
												if(itemDC.getOldValue().equals(value)){//匹配
