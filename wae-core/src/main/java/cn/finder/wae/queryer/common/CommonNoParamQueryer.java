 ...
 *
 */
public class CommonNoParamQueryer extends  BaseCommonDBQueryer{

	
	private final static Logger logger = Logger.getLogger(CommonNoParamQueryer.class);
	
	@Override
	public TableQueryResult queryTableQueryResult(long showTableConfigId,
			QueryCondition<Object[]> condition) {
		
		logger.debug("=====go into CommonNoParamQueryer======");
	  ...
	  //mysql 
	    ...
	    String tmp_sql = sql_sb.toString();
	    String tmp_cnt_sql = sql_cnt_sb.toString();
	    ...
	    if(params!=null && params.length>0){
			...
			{
	    		tmp_sql=tmp_sql.replaceFirst("\\?", (String)params[i]);
	    		tmp_cnt_sql=tmp_cnt_sql.replaceFirst("\\?", (String)params[i]);
	    		
			}
	    }
	    
	    
	    logger.debug("== 查询数据SQL："+tmp_sql);
	    ...
	    List<Map<String, Object>>  dataList=getJdbcTemplate().query(tmp_sql,new RowMapper<Map<String,Object>>(){
	    ...
	    logger.debug("== 查询记录数SQL："+tmp_cnt_sql);
	    
	   long cnt = queryForLong(tmp_cnt_sql);
