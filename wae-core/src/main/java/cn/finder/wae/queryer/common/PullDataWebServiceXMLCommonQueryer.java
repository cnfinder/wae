...
/***
 *  定时拉取WEB服务数据  (应由 任务计划执行) XML格式   /students/student
 * @author jcl
 *
 */
public class PullDataWebServiceXMLCommonQueryer extends  BaseCommonDBQueryer{

	
	private final static Logger logger = Logger.getLogger(PullDataWebServiceXMLCommonQueryer.class);
	...
	public TableQueryResult queryTableQueryResult(long showTableConfigId,
	   ...
	   List<ShowDataConfig> showdataConfigs = showTableConfig.getShowDataConfigs();
	    ...
	    try {
			...
			List<Element> list = doc.selectNodes(dataNav);
			...
			for(int i=0;i<list.size();i++){
			    ...
			    Map<String,Object> dataItem = new HashMap<String,Object>();
				   ...
				   for(ShowDataConfig sdf:showdataConfigs){
					   ...
					   if(itemData.element(fieldName)!=null){
						   //接口记录中 包含 配置列
						   
						   Object fieldValue = itemData.element(fieldName).getText();
