...
public class MongoAddCommonQueryer extends  MongoDBQueryer{
	
	private Logger logger =Logger.getLogger(MongoAddCommonQueryer.class);
...
//				value = (new Date());
				...
				if(!StringUtils.isEmpty(value.toString())){
					...
					mongoValues.put(fieldName, t_value);//mongo
				}
			}
			//密码控件，md5加密
...
//					values.add(foreignKey);
			...
			//需要二进制存储列类型的
				...
				mongoValues.put(fieldName, userName);//mongo
			}
		...
		if(returnInt > 0){
			...
			//	getJdbcTemplate().update(sql, parameters);
				...
				getMongoTemplate().save(mongoValues,collectionName);//mongo
			...
			if(returnInt == 2){
				...
				update.push(map.get("fieldName").toString(),v_key);
				
				
				getMongoTemplate().updateFirst(new Query().addCriteria(new Criteria(primaryKey).is(v_key)), update, collectionName);
			}
			return v_key;
		}else{
			Object[] parameters = values.toArray();
