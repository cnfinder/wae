 ...
 *
 */
public class EditCascadeMultiKeyCommonQueryer extends  BaseCommonDBQueryer{
	...
	public TableQueryResult queryTableQueryResult(long showTableConfigId,
		...
		logger.debug("=====go into EditCascadeMultiKeyCommonQueryer======");
	...
	public int singlEditRecord(long editConfigId, String tableName, Map<String, Object> map ){
		...
		//主键列配置
		//List<ShowDataConfig> primaryKeyConfigs =ArchCache.getInstance().getShowTableConfigCache().getPKShowDataConfigs(editConfigId);
		
		
		//WHERE条件
		StringBuffer sb_where =new StringBuffer();
		sb_where.append(" where  1=1");
		...
		for(ShowDataConfig showDataConfig:showDataConfigs){
			...
			if(showDataConfig.getIsPrimaryKey() == 1){        //判断是否是主键
				...
				sb_where.append(" and ").append(fieldName).append("=?");
				where_values.add(value);
