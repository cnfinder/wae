...
public class PropertiesGenerateQueryerBefore extends QueryerDBBeforeClass {
	...
	public void handle(long showTableConfigId,
		...
		logger.debug("====================PropertiesGenerateQueryerBefore.handle ");
	   ...
	   if(count==0){
			if(propertiesTemplateItem!=null && propertiesTemplateItem.size()>0){
				...
				String sql = "insert into "+tableName+"(name,show_type,data_type,process_command,data_showtableConfigId," +
						"data_source_static,prop_name,is_new_line,is_required,sort,business_key,properties_template_type_code,ctrl_width," +
						"ctrl_height,static_default_value,dynamic_default_value,is_editable) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
				for(int i=0;i<propertiesTemplateItem.size();i++){
					...
					int ctrl_width = propitem.getCtrl_width();
					int ctrl_height = propitem.getCtrl_height();
					String static_default_value = propitem.getStatic_default_value();
					String dynamic_default_value = propitem.getDynamic_default_value();
					int is_editable = propitem.getIs_editable();
					
					getJdbcTemplate().update(sql, new Object[]{name,show_type,data_type,process_command,data_showtableConfigId,data_source_static,
							prop_name,is_new_line,is_required,sort,business_key,propertiesTemplateTypeCode,ctrl_width,ctrl_height,static_default_value,dynamic_default_value,is_editable});
				}
			}
	   }
