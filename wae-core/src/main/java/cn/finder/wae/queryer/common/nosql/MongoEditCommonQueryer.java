 ...
 * 
 */
public class MongoEditCommonQueryer extends MongoDBQueryer {
	...
	public TableQueryResult queryTableQueryResult(long showTableConfigId, QueryCondition<Object[]> condition) {
		...
		logger.debug("=====go into MongoEditCommonQueryer======");
		...
		// DataSource ds =
		// DSManager.getDataSourceMaps().get(showTableConfig.getTargetDs());
	...
	public int singlEditRecord(long editConfigId, String tableName, Map<String, Object> map) {
		...
		String collectionName = tableName;// mongo
		Document mongoValues = new Document();// mongo
		...
		for (ShowDataConfig showDataConfig : showDataConfigs) {
			...
			if (showDataConfig.getShowType() == ConstantsCache.ControlType.CONTROLTYPE_SYSTEM_DATE) {
				...
				mongoValues.put(fieldName, new Timestamp(System.currentTimeMillis()));// mongo
				continue;
			}
			// 需要二进制存储列类型的
			if (showDataConfig.getShowType() == ConstantsCache.ControlType.CONTROLTYPE_BINARY_FILE) {
				...
				if (value instanceof byte[]) {
					...
					valueSql += fieldName + " = ?, ";
					values.add(value);
					mongoValues.put(fieldName, value);// mongo
					continue;
				} else {
					// 根据相对路径获取绝对路径+
			...
			/**
			 * 1.判断文件路径是否为空 2.获取当前文件类型 3.获取主键拼接 主键.类型文件 4.将文件改名 5.设置文件路径属性字段值
			 */
			...
			if (value != null && showDataConfig.getShowType() != ConstantsCache.ControlType.CONTROLTYPE_PWD_CONFIRM) { // 33不是需要插入到数据库的类型
				if (value.equals("")) {
					if (showDataConfig.getShowType() != ConstantsCache.ControlType.CONTROLTYPE_PWD) {
						if (showDataConfig.getDataType() != ConstantsCache.DataType.DATATYPE_STRING) {
							if (!StringUtils.isEmpty(showDataConfig.getDefaultValue())) {
								...
								// if(showDataConfig.getShowType() ==
								// ConstantsCache.ControlType.CONTROLTYPE_DATE){
								// value = null;
								// }
					...
					if (showDataConfig.getShowType() == ConstantsCache.ControlType.CONTROLTYPE_PWD) {
						...
						mongoValues.put(fieldName, MD5Util.getMD5(value.toString()));// mongo
					} else {
						valueSql += fieldName + " = ?, ";
						values.add(value);
						mongoValues.put(fieldName, value);// mongo
		...
		BasicDBObject primary = new BasicDBObject(primaryKey, primaryKeyValue);

		if (primaryKeyValue == null || StringUtil.isEmpty(String.valueOf(primaryKeyValue))) {
			logger.error("error:字段所在表配置错误");
			return -4;// 主键设置错误！
		}

		// int iValue = getJdbcTemplate().update(sql, values.toArray());

		long iValue = getMongoTemplate().getCollection(collectionName).updateOne(primary, new BasicDBObject("$set", mongoValues)).getModifiedCount();
		return (int)iValue;
	}
