...
/***
 *与CommonQueryer基本相同， 可以处理showtable中sql条件配置group by
 * @author wanggan
 *
 */
public class ComGroupQueryer extends  BaseCommonDBQueryer{

	
	private final static Logger logger = Logger.getLogger(ComGroupQueryer.class);
	   ...
	   final  List<String> filedNames=new ArrayList<String>();
	    ...
	    if(StringUtils.isEmpty(condition.getWhereCluster()))
	    {
	    	...
	    	if(!StringUtils.isEmpty(showTableConfig.getSqlConfig())){
	    		...
	    		else if(showTableConfig.getSqlConfig().toLowerCase().startsWith("group")){
	    			where_sb.append("  "+showTableConfig.getSqlConfig());
	    		}
	    		else
	    ...
	    sql_sb.append("select ").append(fields_sb+" ").append(" from ").append(showTableConfig.getShowTableName()).append(" alias ").append(" where 1=1 ");
	    
	    sql_cnt_sb.append("select ").append(" count(*) cnt ").append(" from ").append(showTableConfig.getShowTableName()).append(" alias ").append(" where 1=1 ");
	    //oracle
	    ...
	    if(!StringUtils.isEmpty(where_sb.toString())){
	    	if(where_sb.toString().trim().toLowerCase().startsWith("order by") || where_sb.toString().trim().toLowerCase().startsWith("group by") )
	    	{
