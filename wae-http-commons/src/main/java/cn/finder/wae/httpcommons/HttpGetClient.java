...
public class HttpGetClient extends cn.finder.httpcommons.HttpGetClient {
	...
	public HttpGetClient(String url,String appKey, String appSecret) {
		super(url);
		this.appKey = appKey;
	...
	public HttpGetClient(String serverUrl, String format,String appKey, String appSecret)
    {
		super(serverUrl,format);
    }
    public HttpGetClient(String serverUrl, String format,boolean disableParser,String appKey, String appSecret)
    {
    ...
    public  <T extends ApiResponse>  T doExecute(BaseRequest<T> request, String session, Date timestamp)
    {
        ...
        try
        {
            ...
            {//  二进制(文件、图片)请求
                ...
                retStr = this.webUtils.doPostMultipart(this.url, parameters, fileParams);
            }
            ...
            {
                //发送普通文本请求
                retStr = this.webUtils.doGet(this.url, parameters);
